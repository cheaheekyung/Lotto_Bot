...
def main_view(request):
    ...
    context = {"latest_numbers": latest_numbers}
    return render(request, "lottobot/main.html", context)


class MainpageAPIView(
    APIView
):  # 메인페이지 로그인해야 볼수있게할건지?? 거기에맞게 수정만하면됨 일단 다 가능하게해둠
...
class MypageAPIView(APIView):
    ...
    def get(self, request):
        ...
        for recommendation in recommendations:
            ...
            lotto_draw = (
                LottoDraw.objects.filter(
                    draw_date__gte=recommendation.created_at.date()
                )
                .order_by("draw_date")
                .first()
            )
            ...
            draw_date = (
                lotto_draw.draw_date
                if lotto_draw
                else "아직 추천이후 최신회차가 진행되지 않았습니다."
            )
            ...
            if lotto_draw:
                ...
                winning_numbers = set(
                    lotto_draw.winning_numbers.replace(" ", "").split(",")
                )
